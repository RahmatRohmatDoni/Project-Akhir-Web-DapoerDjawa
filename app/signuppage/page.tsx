...
export default function SignUpPage() {
  ...
  // Handler untuk validasi input Nama (hanya mengizinkan huruf dan spasi)
  ...
  // Handler untuk pendaftaran manual dengan Email & Password
  const handleSignUp = (e: React.FormEvent) => {
    ...
    if (!name || !email || !password) {
      return setModalConfig({
        ...
        message: "Semua kolom (Nama, Email, dan Password) wajib diisi.",
        redirectOnClose: false, 
      });
    }
    ...
    localStorage.setItem("dapoerdjawa_registered_user", JSON.stringify(userData));

    // Menampilkan modal pendaftaran sukses
    setModalConfig({
      isOpen: true,
      isError: false,
      title: "Pendaftaran Berhasil!",
      message: `Akun atas nama ${name} berhasil dibuat. Silakan login untuk melanjutkan belanja.`,
  ...
  // --- INTEGRASI SIGN UP GOOGLE FIREBASE ---
  const handleGoogleSignup = async () => {
    setIsGoogleLoading(true);

    // Workaround: Menangkap event jika user menutup pop-up Google di luar jendela browser secara manual
    ...
    try {
      ...
      if (
        ...
        setIsGoogleLoading(false); 
        return; // Menghentikan loading secara transparan tanpa error (Silent Exit)
      }
      ...
      setModalConfig({
        ...
        isError: true,
        title: "Pendaftaran Gagal",
        message: "Proses pendaftaran dengan Google gagal atau terjadi kesalahan jaringan.",
        redirectOnClose: false, 
      });
    } finally {
      setIsGoogleLoading(false);
    ...
    <section className="w-full bg-gray-50 text-gray-800 leading-normal text-base tracking-normal min-h-screen flex flex-col pt-24">
      ...
      <main className="w-full flex-1 flex items-center justify-center p-6 my-10">
        <div className="max-w-md w-full bg-white rounded-2xl shadow-xl p-8 border border-gray-100 relative overflow-hidden">
          ...
          <button onClick={handleGoogleSignup} type="button" className="w-full border border-gray-300 text-gray-700 font-medium py-2.5 rounded-lg hover:bg-gray-50 transition-colors flex justify-center items-center gap-2">
          ...
          <div className="mt-8 text-center text-sm text-gray-600">
            Already have an account? <Link href="/loginpage" className="font-semibold text-black hover:underline">Login</Link>
          </div>
        </div>
      </main>
